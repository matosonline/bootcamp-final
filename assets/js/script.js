...
$(document).ready(function(){
    ...
    //READ CITY FOR FORMS    
        var curl = '../controllers/City_read.php';
    $.ajax({
        type        : 'GET', // define the type of HTTP verb we want to use (POST for our form)
        url         :  curl, // the url where we want to POST
    ...
    //READ ROLE FOR FORMS    
        var rurl = '../controllers/Role_read.php';
        $.ajax({
            type        : 'GET', // define the type of HTTP verb we want to use (POST for our form)
            url         :  rurl, // the url where we want to POST
            dataType    : 'json' // what type of data do we expect back from the server
        })  .done(function(data){
                console.log(data);
...
$(document).on('click','#btn-save',function(e) {
    ...
    var url = 'holycrapteamproject-rickmatos.c9users.io/User_signup.php';
    $.ajax({
        data: data,
        type: "POST",
        url: url
